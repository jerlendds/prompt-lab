...
export default function SettingsModal({
  ...
  function importAny() {
    ...
    reader.onload = function (event) {
      ...
      if (
        !checkProperties(newPrompts[0], ["title", "text", "description", "folder", "tags", "id"])
      ) {
            ...
            <div className="flex-grow overflow-hidden">
              <ul className="tabs tabs-lifted w-full flex justify-between">
                <a
                  ...
                  className={`p-1 grow tab ${currentPage === "General" ? "tab-active" : ""}`}
                >
                  <div className={"pb-3"}>{t(k.GENERAL_SETTINGS)}</div>
                </a>
                <a
                  onClick={() => handlePageChange("Folders")}
                  className={`p-1 grow tab ${currentPage === "Folders" ? "tab-active" : ""}`}
                >
                  {t(k.MANAGE_FOLDERS)}
                </a>
                <a
                  onClick={() => handlePageChange("Export")}
                  className={`p-1 grow tab ${currentPage === "Export" ? "tab-active" : ""}`}
                >
                  {t(k.IMPORT_EXPORT)}
                </a>
                <a
                  onClick={() => handlePageChange("Cloud")}
                  className={`p-1 grow tab ${currentPage === "Cloud" ? "tab-active" : ""}`}
                >
                  {t(k.CLOUD_SYNCING)}
                </a>
              </ul>
            ...
            {currentPage === "General" && (
              <div className="card mt-3 mb-3">
                <div className="card-body pt-2">
                  ...
                  <div className={"mt-3"}>
                    <div className={"mb-2"}>
                      <h2 className={"text-xl font-semibold my-3"}>{t(k.OTHER_SETTINGS)}</h2>
                      <div className={"flex items-center"}>
                        <h4 className={"text-lg font-medium"}>{t(k.PERSIST_VARIABLES)}</h4> &nbsp;
                        <input
                          id={"persist-toggle"}
                          type="checkbox"
                          className="toggle"
                          defaultChecked={localStorage.getItem("persist_variables") === "true"}
              ...
              <>
                ...
                <div className="card mt-3 mb-3">
                  <div className="card-body pb-1 pt-1">
                    ...
                    <p>
                      {t(k.USE_THIS)}{" "}
                      <a className="link link-primary" onClick={downloadTemplate}>
                        {t(k.CSV_TEMPLATE)}
                      </a>{" "}
                      {t(k.TO_IMPORT_PROMPTS)}
                    </p>
                    <button className="m-2 btn" onClick={openFileSelect}>
                      <label id="import-label" className="clickable" htmlFor="import-any">
                        {t(k.IMPORT_CSV)}
                      </label>
                      <input
                        onChange={importAny}
                ...
                <div className="card mt-3 mb-3">
                  <div className="card-body pt-0">
                    ...
                    {confirmDelete && (
                      <button onClick={deletePrompts} className="btn bg-warning">
                        {t(k.CONFIRM_DELETE)}
                      </button>
                    )}
                  </div>
